                                ...
                                <div class="col-md-12 col-xs-12 col-label <?php if(($cont%2)!=0):?> color-struct-contet <?php endif; ?> <?php if($datos['array_contador_preguntas'][$key->formseccion_id]==$datos['contador_respuesta_individual']):?> struct-border-end m-b-10 <?php endif;?>">
                                    ...
                                    <?php for($i=0; $i<count($datos['formrespuestas'][$key->id]); $i++): ?>
                                        <div class="col-md-12 p-l-0 p-r-0">
                                            ...
                                            <div class="col-md-3 msjextra <?php if($i!=0): ?>m-t-10<?php endif; ?><?php if(empty($datos['formrespuestas'][$key->id][$i]['descripcion'])):?>display-none<?php endif; ?> <?php if(isset($datos['formrespuestas'][$key->id][$i]['value'])): ?>display-none<?php endif; ?>"><?=$datos['formrespuestas'][$key->id][$i]['descripcion']?></div>
                                            <div class="col-md-2 txt-align-r version-color <?php if(!isset($datos['formrespuestas'][$key->id][$i]['value'])): ?>m-t-29 display-none<?php endif; ?> <?php if($i==0): ?>m-t-20<?php else :?>m-t-5<?php endif; ?>">
                                            ...
                                            <div class="col-md-2 m-b-10 <?php if(!isset($datos['formrespuestas'][$key->id][$i]['version'])): ?>m-t-25 display-none<?php endif; ?> <?php if($i==0): ?>m-t-16<?php else :?>m-t-10<?php endif; ?>"><a id="ver_<?=$key->id?>_<?=$datos['formrespuestas'][$key->id][$i]['id']?>" class="btn-ver m-r-11" target="_blank" <?php if(isset($datos['formrespuestas'][$key->id][$i]['version'])):?> href="<?=$real_url."showFile/".$datos['formrespuestas'][$key->id][$i]['version'][count($datos['formrespuestas'][$key->id][$i]['version'])-1]['id']?>"<?php else: ?>href="#"<?php endif; ?>>Ver</a><a id="download_<?=$key->id?>_<?=$datos['formrespuestas'][$key->id][$i]['id']?>" class="btn-ver" <?php if(isset($datos['formrespuestas'][$key->id][$i]['version'])):?> href="<?=$real_url."download/".$datos['formrespuestas'][$key->id][$i]['version'][count($datos['formrespuestas'][$key->id][$i]['version'])-1]['id']?>" <?php else: ?>href=""<?php endif; ?>>Descargar</a></div>
                                            <?php if($i==0): ?>
                                                <div class="col-md-3 pull-right p-r-0 <?php if(isset($datos['formrespuestas'][$key->id][$i]['version'])): ?> <?php if($i==0): ?>m-t-9<?php else: ?>m-t-2<?php endif;?> <?php else: ?>m-t-23<?php endif; ?>">
                                                    <div class="input textarea">
                                                        <?php if($congelado==0): ?>
                                                            <div class="content-answer"><i class="fa fa-list-ul form-control-feedback new-position"></i><textarea class="text-obs" style="width:100%;" name="input_<?=$key->id?>_<?=$datos['formrespuestas'][$key->id][$i]['id']?>"
                                                                                                                                                                  id="input_<?=$key->id?>_<?=$datos['formrespuestas'][$key->id][$i]['id']?>"
                                                                                                                                                                  onblur="setData(<?=$key->id.",".$datos['formrespuestas'][$key->id][$i]['id']?>)"
                                                    ...
                                                    <?php if(isset($datos['formrespuestas'][$key->id][$i]['observ'])): ?>
                                                        <div class="list-observ">
                                                            <ol>
                                                                <?php for($fob=0; $fob<count($datos['formrespuestas'][$key->id][$i]['observ']); $fob++): ?>
                                                                    <li class="styleobs"><?=$datos['formrespuestas'][$key->id][$i]['observ'][$fob]['value']?></li>
                                                                <?php endfor; ?>
                                                            </ol>
                                                        </div>
                                                    <?php endif; ?>
...
<script>
    ...
    function setData(val,i) {
        if($("#input_"+val+"_"+i).val().trim().length!=0){
            var res=savedataobserv(val,$("#input_"+val+"_"+i).val().trim(),iddataform,i);//ajax subida de datos cargadatos.js
            if(res['res']){
                $("#alert-savedata-"+val).removeClass("alert-danger");
                $("#alert-savedata-"+val).addClass("alert-success");
                $("#msj-savedata-"+val).text("Observación Guardada Exitosamente");
            }
            else{
                $("#alert-savedata-"+val).removeClass("alert-success");
                $("#alert-savedata-"+val).addClass("alert-danger");
                $("#msj-savedata-"+val).text("No se ha podido guardar la observación");
            }
            $("#alert-savedata-"+val).slideDown();

            setTimeout(function () {
                $("#alert-savedata-"+val).slideUp();
            }, 4000);
        }
    }

    function setTypesave(val){
        var url = getUrl() + "changeStateWf/" +val+"/observations";
    ...
    jQuery(function(){
        $('.fsiguiente').click( function(){
            ...
            var url = getUrl() + "observations/"+"<?=$_SESSION['fd_if_formdinamic']."/".$_SESSION['fd_id_contacto']."/"?>"+iddataform+"/next";
            window.location.assign(url);
        });

        $('.fanterior').click( function(){
            var idformdinamic="<?=$_SESSION['fd_if_formdinamic']?>";
            var url = getUrl() + "observations/"+"<?=$_SESSION['fd_if_formdinamic']."/".$_SESSION['fd_id_contacto']."/"?>"+iddataform+"/previous";
            window.location.assign(url);
        });
